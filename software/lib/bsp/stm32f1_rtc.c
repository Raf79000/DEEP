...
void RTC_set_alarm(RTC_TimeTypeDef * alarm_time, bool_e enable_interrupt, bool_e * flag)
{
	if(initialized)
	{
		...
		if(enable_interrupt)
		{
			...
			if(flag)
				alarm_interrupt_flag = flag;	//on sauvegarde l'adresse du flag pour qu'il soit lev� lors de l'interruption provoqu�e par l'alarme.
		}
		else
...
/*
 * Cette fonction permet de modifier la vitesse de la RTC (� des fins de tests notamment)
 * @param time_acceleration : 1 pour une vitesse normale. 60 pour un �coulement d'une minute par seconde. 5*60 pour 5mn/seconde...
